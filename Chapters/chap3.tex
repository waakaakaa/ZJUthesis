...
这个模版的目的就是让没有一点\LaTeX基础的同学也能很快地用\LaTeX组织出自己的论文。
...
就是这个模版文件的文件名。
而{\bf$\backslash$documentclass}则是一个命令，在\LaTeX源文件中，
...
你可以按上面说的把这个说明文档改成双面模式，然后保存，再运行makethesis.bat，生成出来的“论文模版示例.pdf”\footnote{生成新的“论文模版示例.pdf时，如果该文件正被打开，请先关闭。否则不会生成新的文件。”}，看看跟单面模式有何不同？
...
其中的 $\backslash$hsapce\{{\bf1.5em}\} 表示空间间距为{\bfseries一个半字符}，
如果要{\bfseries一个字符}间距，则写{\bf 1em}\footnote{$\backslash$hsapce\{1em\}也可以写作$\backslash$quad。}，{\bfseries两个字符}的间距，则是{\bf 2em}，以此类推。
...
然后{\bfseries再}点击“LaTeX”命令{\bfseries两次}，最后再进行dvi到pdf的转换，
...
这里{\bfseries不要忘记}还有{\bfseries版本信息页}

\vspace{8pt}
\verb+\makeOSandCPRTpage+
\vspace{8pt}
...
\LaTeX的文章内容书写没有什么特别的格式，直接书写即可。
但需要注意以下几点：

\begin{enumerate}
\item{\LaTeX会忽略中文字符间的空格，也就是说：}
...
但在英文单词前后的空格，\LaTeX会{\bfseries保留一个}。

\item{断行与分段}

\LaTeX中，{\bfseries单独一个回车}也会像空格一样被忽略，空行作为分段的标志，
...
那么如果不做一点特殊处理，\LaTeX在生成PDF时可能会出现某一行最后一个英文单词突出行外。
解决这个问题的办法就是在英文字符、单词的前面及后面加上空格符号。
当然，这样不太符合我们自己的写作习惯，一个个加起来也太费事。
没关系，\LaTeX的中文先驱们已经替我们简化了这个问题。
...
以上就是书写论文正文内容时的一些说明，更多常见问题，请参考《一份不太简短的\LaTeXe介绍》\cite{LaTeXshzh}和《CTeX FAQ》。
...
就是说\LaTeX在编译这份tex源文件时读到这里，会去找这里指定的另一个tex文件，
并把它的内容全部复制到这个位置。
...
这一部分只需要如下一个命令，其它的东西\LaTeX自己会替你搞定。
...
这一部分同样只需要如下一个命令，其它的东西\LaTeX会替你做好。
...
在开始正文内容之前，需要一个\LaTeX命令来告诉软件进入正文部分，
页码需要重新从1编号，并使用阿拉伯数字，以及章节开始从1开始。这个命令是

\verb+\ZJUmainmatter+

正文会有很多章，概述，条件，分析，讨论，结论什么的。编写tex文件的时候，
需要给\LaTeX明确哪些是章，哪些是节，哪一些是更小的节。这样，\LaTeX才能正确地给你的正文内容分章，分节
...
…………
\end{verbatim}
}csi.miami.505.hdtv-lol

章节标题中不需要说明是第几章第几节，\LaTeX会替你计算，
你所要做的只是把你的文章按结构统一起来即可。
并且，目次也会根据你的标题内容自动生成。
这里要补充一点的是：有的标题可能会比较长，写到目录里会导致目录条目换行，
为解决这个问题，\LaTeX允许目录中的标题与实际的标题不一样，要达到这个效果，
...
附录章节内容写法与正文写法完全相同，\LaTeX会替你处理好章节编号等问题，
你完全不用考虑附录的编号。
